...
public class Table implements BacktrackingIterable<Record> {
    ...
    private boolean escalateDisabled;
...
//        enableAutoEscalate();
    ...
    public synchronized RecordId addRecord(List<DataBox> values) {
        if(!escalateDisabled) enableAutoEscalate();
    ...
    public synchronized Record updateRecord(List<DataBox> values, RecordId rid) {
        ...
        LockContext ctxt = lockContext.childContext(rid.getPageNum());
//        if(ctxt.getExplicitLockType(TransactionContext.getTransaction())==LockType.NL ||
//                !ctxt.getExplicitLockType(TransactionContext.getTransaction()).equals(LockType.X)){
            LockUtil.ensureSufficientLockHeld(ctxt,LockType.X);
//        }
...
//        }
    ...
    public void enableAutoEscalate() {
        ...
        if(lockContext!=null){
            double pageNum = lockContext.capacity();
            double locksHeldNum = lockContext.saturation(TransactionContext.getTransaction());
//        System.out.println(pageNum);
//        System.out.println(locksHeldNum);
            if(locksHeldNum >= 0.2 && pageNum >= 10){

                lockContext.escalateTable(TransactionContext.getTransaction());
            }
        }
