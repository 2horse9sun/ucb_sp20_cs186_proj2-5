...
//        System.out.println(lockType);
        ...
        if(readonly && (lockType==LockType.X||lockType==LockType.IX)) throw new UnsupportedOperationException("Read only");
        LockType pType;
        if(parent != null){
            pType = parent.getExplicitLockType(transaction);
        }else{
            pType = LockType.NL;
        }
        if(parent==null || LockType.substitutable(pType,LockType.parentLock(lockType))){
    ...
    public void delNumChildLocks(TransactionContext transaction, LockContext parent){
        if(parent == null) return;
        int num = parent.numChildLocks.getOrDefault(transaction.getTransNum(), 0);
        parent.numChildLocks.put(transaction.getTransNum(), num - 1);
    ...
    throws NoLockHeldException, InvalidLockException {
        ...
        if(!numChildLocks.keySet().contains(transNum) || numChildLocks.get(transNum)==0){
    ...
    throws DuplicateLockRequestException, NoLockHeldException, InvalidLockException {
        // TODO(proj4_part2): implement
        if(readonly) throw new UnsupportedOperationException("Read only");
//        System.out.println(name + " " +newLockType);
        List<ResourceName> released = new ArrayList<>();
        released.add(name);
        if(newLockType == LockType.SIX) lockman.acquireAndRelease(transaction,name,newLockType,released);
    ...
    public void escalate(TransactionContext transaction) throws NoLockHeldException {
        ...
        if(readonly) throw new UnsupportedOperationException("Read only");
        Long transNum = transaction.getTransNum();
        if(getExplicitLockType(transaction) == LockType.NL) throw new NoLockHeldException("No lock held");
        if(getExplicitLockType(transaction) == LockType.S) return;
        if(getExplicitLockType(transaction) == LockType.X && numChildLocks.getOrDefault(transNum,0) == 0) return;
        ...
        for(Lock lock: lockman.getLocks(transaction)){
            ...
            if(!isChild(name,lock.name)) continue;
            if(lock.lockType!= LockType.IS && lock.lockType!=LockType.S) shouldUpgrade = true;
            if(lock.lockType == LockType.X) hasX = true;
            LockContext cxt = fromResourceName(lockman,lock.name);
            releasedLocks.add(lock.name);
            delNumChildLocks(transaction,cxt);
        }
        if(hasX){
            for(Lock lock: lockman.getLocks(transaction)){
                if(lock.lockType == LockType.X) releasedLocks.remove(lock.name);
            }
        }


        if(shouldUpgrade){
            if(hasX) lockman.acquireAndRelease(transaction,name,LockType.SIX,releasedLocks);
            else lockman.acquireAndRelease(transaction,name,LockType.X,releasedLocks);
            addNumChildLocks(transaction,this);
        }else{
//            lockman.acquireAndRelease(transaction,name, LockType.SIX,releasedLocks);
    ...
    public void escalateTable(TransactionContext transaction) throws NoLockHeldException {
        ...
        List<Lock> locksTable = new ArrayList<>();

        for(Lock lock : locks){
            if(lock.name.isDescendantOf(name) || lock.name==name) locksTable.add(lock);
        }

        for(Lock lock: locksTable){
                if(!isChild(name,lock.name)) continue;
                if(lock.lockType!= LockType.IS && lock.lockType!=LockType.S) shouldUpgrade = true;
                if(lock.lockType == LockType.X) hasX = true;
                if(lock.lockType == LockType.S) hasS = true;
                LockContext cxt = fromResourceName(lockman,lock.name);
                if(lock.name.isDescendantOf(name) || lock.name==name) releasedLocks.add(lock.name);
                delNumChildLocks(transaction,cxt);

        }
        if(hasX && hasS){
            for(Lock lock: locksTable){
                if((lock.name.isDescendantOf(name)||lock.name==name) && lock.lockType == LockType.X) releasedLocks.remove(lock.name);
            }
        }
        ...
        if(shouldUpgrade){
            if(hasX && hasS) lockman.acquireAndRelease(transaction,name,LockType.SIX,releasedLocks);
            else lockman.acquireAndRelease(transaction,name,LockType.X,releasedLocks);
            addNumChildLocks(transaction,this);
        }else{
//            lockman.acquireAndRelease(transaction,name, LockType.SIX,releasedLocks);
    ...
    public boolean isChild(ResourceName parent, ResourceName child){
        LockContext pCxt = fromResourceName(lockman, parent);
        LockContext cCxt = fromResourceName(lockman, child);
        if (pCxt.parent == cCxt.parent) {
            return true;
        }
        if (cCxt.parent == null) {
            return false;
        }
        return isChild(parent, cCxt.parent.name);
    }
    ...
    public LockType getEffectiveLockType(TransactionContext transaction) {
        ...
        if (type == LockType.NL && parent != null) {
