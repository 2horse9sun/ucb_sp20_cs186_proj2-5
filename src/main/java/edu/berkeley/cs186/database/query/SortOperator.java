...
public class SortOperator {
    ...
    public Run sortRun(Run run) {
        ...
        IntermediateRun newRun = new IntermediateRun();
        newRun.addRecords(recordList);
        return newRun;
    }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for(Run run : runs) iters.add(run.iterator());
        for(int i = 0;i < k;++i){
            q.add(new Pair<Record,Integer>(iters.get(i).next(),i));
        }
        while(!q.isEmpty()){
            Pair<Record,Integer> min = q.poll();
            Record minRecord = min.getFirst();
            int minIndex = min.getSecond();
            recordList.add(minRecord);
            Iterator<Record> iter = iters.get(minIndex);
            if(iter.hasNext()){
                q.add(new Pair<Record,Integer>(iter.next(),minIndex));
            }
        }
        IntermediateRun newRun = new IntermediateRun();
        newRun.addRecords(recordList);
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int index = 0;
        while(index < N){
            List<Run> tmp = new ArrayList<>();
            for(int i = 0;i < numBuffers-1;++i){
                if(index < N){
                    tmp.add(runs.get(index));
                    index++;
                }
                else break;
            }
            mergedSortedRuns.add(mergeSortedRuns(tmp));
        }
    ...
    public String sort() {
        ...
        while(pageIterator.hasNext()){
            BacktrackingIterator<Record> blockIterator = transaction.getBlockIterator(tableName,pageIterator,numBuffers);
