...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        pass1Map.put(set,minCostSingleAccess(startTableName));
        for(String tableName : joinTableNames){
            set = new HashSet();
            set.add(tableName);
            pass1Map.put(set,minCostSingleAccess(tableName));
        }
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        int minCostIndex = -1;
//        int minIndexScanCost = Integer.MAX_VALUE;
        for(int eligibleIndex : eligibleIndexColumns){
            QueryOperator indexScanOp = new IndexScanOperator(transaction,table,selectColumnNames.get(eligibleIndex),selectOperators.get(eligibleIndex),selectDataBoxes.get(eligibleIndex));
            int cost = indexScanOp.estimateIOCost();
            if(cost < minCost){
                minCost = cost;
                minOp = indexScanOp;
                minCostIndex = eligibleIndex;
            }
        }
...
//        if(seqScanCost < minIndexScanCost){
//            minOp = seqScanOp;
//        }
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        for(Set tableSet : prevMap.keySet()){
            for(int i = 0; i < joinTableNames.size();++i){
                ...
                if(tableSet.contains(leftSide[0]) && !tableSet.contains(rightSide[0])) {
                    newSet = new HashSet(Collections.singleton(rightSide[0]));
                    leftOp = prevMap.get(tableSet);
                    rightOp = pass1Map.get(newSet);
                }else if(!tableSet.contains(leftSide[0]) && tableSet.contains(rightSide[0])) {
                    newSet = new HashSet(Collections.singleton(leftSide[0]));
                    leftOp = pass1Map.get(newSet);
                    rightOp = prevMap.get(tableSet);
                }else{
                    continue;
                }
                minJoinOp = minCostJoinType(leftOp, rightOp, leftSide[1], rightSide[1]);
                newSet.addAll(tableSet);
                if (!map.containsKey(newSet)) {
                    map.put(newSet, minJoinOp);
                } else if (minJoinOp.getIOCost() < map.get(newSet).getIOCost()) {
                    map.put(newSet, minJoinOp);
                }
            }
        }
