...
//        if(lockContext.getExplicitLockType(TransactionContext.getTransaction()) == LockType.NL ||
//                !lockContext.getExplicitLockType(TransactionContext.getTransaction()).equals(LockType.X)){
//            LockUtil.ensureSufficientLockHeld(lockContext,LockType.X);
//        }
    }
...
//<<<<<<< HEAD
        BPlusTreeIterator treeIterator = new BPlusTreeIterator(root, root.getLeftmostLeaf().getKeys().get(0));
        return treeIterator;
...
//        }
        ...
        // TODO(proj2): Return a BPlusTreeIterator.
//<<<<<<< HEAD
        BPlusTreeIterator treeIterator = new BPlusTreeIterator(root, key);
        return treeIterator;
...
//<<<<<<< HEAD
        try{
            ...
            if(!splitNode.isPresent()) return;
            DataBox splitKey = splitNode.get().getFirst();
            Long splitPageNum = splitNode.get().getSecond();
            List<DataBox> keys = new ArrayList<>();
            keys.add(splitKey);
...
//<<<<<<< HEAD
        while(data.hasNext()){
            ...
            if(!splitNode.isPresent()) continue;
            DataBox splitKey = splitNode.get().getFirst();
            Long splitPageNum = splitNode.get().getSecond();
            List<DataBox> keys = new ArrayList<>();
            keys.add(splitKey);
...
//        }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private BPlusNode root;
        private DataBox currKey;
        private LeafNode currNode;
        private List<RecordId> currRids;
        private Iterator<RecordId> currRidsIterator;

        private BPlusTreeIterator(BPlusNode root, DataBox currKey) {
            this.root = root;
            this.currKey = currKey;
            this.currNode = root.get(currKey);
            int index = InnerNode.numLessThan(currKey, currNode.getKeys());
            this.currRids = currNode.getRids().subList(index, currNode.getRids().size());
            this.currRidsIterator = currRids.iterator();
        }

        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            if(currRidsIterator.hasNext()) return true;
            Optional<LeafNode> rightSibling = currNode.getRightSibling();
            if(!rightSibling.isPresent()) return false;
            currNode = rightSibling.get();
            currRids = currNode.getRids();
            currRidsIterator = currRids.iterator();
            return currRidsIterator.hasNext();
        }
        ...
        public RecordId next() {
            ...
            if(!currRidsIterator.hasNext()) throw new NoSuchElementException();
            return currRidsIterator.next();
        }
    }
}
